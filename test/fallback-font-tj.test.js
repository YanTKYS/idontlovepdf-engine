...
/**
 * The document's own font: a subset, as a real PDF's embedded font is. It knows 令和8年度
 * and a few more characters and NOTHING else -- in particular none of しょうわ, which is
 * what sends a replacement down the fallback path.
 *
 * The widths are deliberately not all 1000. "Japanese is full-width so the widths must
 * match" is exactly the assumption this version is not allowed to make: 和 is 950 and 8 is
 * 500 here, so any rewrite that does not actually do the arithmetic moves the page.
 */
const FONT = new Map([
...
const glyphs = (text) => `<${[...text].map((character) => FONT.get(character).code).join("")}>`;
...
const W_ARRAY = [...FONT.values()].map(({ code, width }) => `${Number.parseInt(code, 16)} [${width}]`).join(" ");
...
/* --------------------------------------------------------------------------------------
 * An independent reader: nothing below imports from src/, so what it says about where a
 * glyph lands is a second opinion, not a restatement of the engine's own.
 * ----------------------------------------------------------------------------------- */

/**
 * Every indirect object's dictionary, keyed by object number, taking the LAST definition of
 * each -- which is what a reader following an incremental update's xref chain arrives at.
 * Located by `<<`/`>>` depth rather than by `endobj`, so the deflated font program's bytes
 * cannot be mistaken for structure.
 */
function dictionariesOf(pdf) {
  const text = latin1.decode(pdf);
  const dictionaries = new Map();
  for (const header of text.matchAll(/(?:^|[\r\n])(\d+) 0 obj\s*(?=<<)/g)) {
    const start = text.indexOf("<<", header.index);
    let depth = 0;
    let cursor = start;
    ...
    dictionaries.set(Number(header[1]), text.slice(start, cursor));
  }
  return dictionaries;
}

/** `/W` -> CID -> width, parsed here rather than by the engine's own reader. */
function cidWidthsOf(dictionary) {
  const opener = /\/W\s*\[/.exec(dictionary);
  const widths = new Map();
  if (opener) {
    let depth = 1;
    let cursor = opener.index + opener[0].length;
    const start = cursor;
    for (; cursor < dictionary.length && depth; cursor += 1) {
      if (dictionary[cursor] === "[") depth += 1;
      else if (dictionary[cursor] === "]") depth -= 1;
    }
    const tokens = dictionary.slice(start, cursor - 1).match(/\[|\]|-?[\d.]+/g) ?? [];
    let index = 0;
  ...
  const defaultWidth = /\/DW\s+(-?[\d.]+)/.exec(dictionary);
  return { widths, defaultWidth: defaultWidth ? Number(defaultWidth[1]) : 1000 };
}

/** Resource name -> how wide each 2-byte code of that font is, read out of the file. */
function fontTable(pdf) {
  const dictionaries = dictionariesOf(pdf);
  ...
  for (const entry of (/\/Font\s*<<([\s\S]*?)>>/.exec(page)?.[1] ?? "").matchAll(/\/([^\s/]+)\s+(\d+)\s+0\s+R/g)) {
    ...
    const descendant = /\/DescendantFonts\s*\[\s*(\d+)\s+0\s+R/.exec(type0 ?? "");
    if (!/\/Subtype\s*\/Type0\b/.test(type0 ?? "")) {
      // A simple font: one byte per code, widths indexed from /FirstChar.
      const first = Number(/\/FirstChar\s+(\d+)/.exec(type0)[1]);
      const table = (/\/Widths\s*\[([^\]]*)\]/.exec(type0)[1].match(/-?[\d.]+/g) ?? []).map(Number);
    ...
    const { widths, defaultWidth } = cidWidthsOf(dictionaries.get(Number(descendant[1])));
 ...
 * text object's origin. Only what these fixtures use is implemented -- 2-byte codes, `Tf`,
 * `Tc`, `Tw`, `Tz`, `Td`, `Tm` translations, `Tj` and `TJ` -- and anything else throws
 * rather than being silently ignored.
 */
function simulate(content, fonts) {

...
test("classifies known engine errors into readable causes and keeps the raw message", () => {
  const cases = [
    ...
    ["replace: This match is drawn as 5 separate text runs, so a replacement of 3 characters cannot be written over 5 without moving text relative to the PDF's own spacing (non-zero-tj-adjustment). Use an equal-length replacement, or an empty one to delete.", "複数runにまたがる一致で、字間調整・書式境界のため文字数を変える置換ができない構造（MULTI_RUN_LENGTH_CHANGE_UNSUPPORTED）（non-zero-tj-adjustment）"],
    ["replace: This match is drawn as 2 separate text runs, so a replacement of 7 characters cannot be written over 6 without moving text relative to the PDF's own spacing (text-state-boundary). Use an equal-length replacement, or an empty one to delete.", "複数runにまたがる一致で、字間調整・書式境界のため文字数を変える置換ができない構造（MULTI_RUN_LENGTH_CHANGE_UNSUPPORTED）（text-state-boundary）"],

...
- **原因は `/Flags` の読み違いではなく、`/FontDescriptor` 自体への到達漏れでした。** `22550.pdf` の `/F3` は、`/DescendantFonts` の CIDFont dictionary を inline で書く（v0.4.3 で対応済み）だけでなく、その CIDFont dictionary 自身の `/FontDescriptor` も inline dictionary として書いていました。v0.5.0 の `fontDescriptorOf()`（`src/font-classification.js`）は `/FontDescriptor N G R` の indirect reference にしか一致せず、inline の場合は「`/FontDescriptor` が全く無い」場合と区別できないまま `unknown`（→ 常に `sans`/BIZ UDゴシック）へ落ちていました。実際には `/Flags` は `6`（Symbolic + Serif）で、Serif bit は明確に立っていました
- **`fontDescriptorOf()` が inline `/FontDescriptor << ... >>` も読むようになりました。** indirect reference の場合は従来どおり resolve し、inline の場合は `src/encryption.js` が Encrypt dictionary の inline `/CF` サブ dictionary を読むのに既に使っている `nestedDictionaryText()`（`src/pdf-dictionary-text.js`）をそのまま再利用します。新しい PDF parser は追加していません。`/Flags` 自身の direct/indirect 読み取りと Serif bit の意味づけは変更していません
- **`classifyFontResource()` の公開シグネチャ（`"serif"`/`"sans"`/`"unknown"` の文字列 3 値）は変更していません。** 内部に `classifyFontResourceDetailed()` を追加し、`unknown` になった理由を developer 診断用に構造化しました（`font-descriptor-missing` / `font-descriptor-unresolved` / `flags-missing` / `flags-unresolved` / `flags-invalid` / `flags-zero` / `serif-flag-set` / `serif-flag-not-set`）。通常利用者向けの公開 error にはしていません
- **`diagnoseFallbackFontSelection(editor, matchId)`（developer/test 専用、`index.js` からは export しない）が `reason`・`fontDescriptor`（`form`・`object`・`text`）・`flags`（`value`・`serifBit`）を追加のフィールドとして返すようになりました。** 既存フィールド（`sourceFontName`・`classification`・`selectedRole`・`availableRoles`）は変更していません
- **開発者向け診断 CLI `scripts/diagnose-font-classification.js` を追加しました。** `node scripts/diagnose-font-classification.js <file.pdf> --text <query>` で、実 PDF の Serif/Sans 判定理由（分類・reason・FontDescriptor の形・`/Flags` 値・Serif bit・embedded font program の有無）を確認できます。読み取り専用で、ネットワークアクセスは一切しません
- **既存の `.github/workflows/diagnose-real-pdf.yml`（manual-only、新規 workflow ではありません）に、分類診断ステップ（常時実行）を追加し、編集 smoke test を `--serif-font` 対応へ拡張しました。** `22550.pdf` に対してこの workflow を実行し、実際に `classification: "serif"`・`selectedRole: "serif"`・埋め込まれた fallback font の `BaseFont` が `BIZUDMincho-Regular` であることを確認しています（run ID [33887555829](https://github.com/YanTKYS/idontlovepdf-engine/actions/runs/33887555829)）。`令和 → 平成`（既存 font 経路、regression）も引き続き成功し、`令和 → しょうわ` は BIZ UD明朝自身の実 glyph 幅で再判定された上で v0.4.4 の overflow 判定により拒否されることを確認しました（fail closed は維持）。後続テキストの描画位置は pdfminer.six で、構造整合性は qpdf で、実際の表示は Chromium の PDF viewer で、いずれも engine と無関係な実装により独立に確認しています
- **`test/font-classification-diagnosis.test.js` を追加しました。** unit レベルで `CLASSIFICATION_REASONS` の全 8 種類を直接確認（fallback font 不要、常時実行）、document レベルで `22550.pdf` の実構造を再現した fixture（`test/helpers/document-font-inline-descriptor.js`）により `令和 → しょ`（serif 選択・BIZ UD明朝埋め込み）・`令和 → 平成`（regression）・`令和 → しょうわ`（実 glyph 幅による再判定）・save → reopen（埋め込んだ BIZ UD明朝自身が再度 serif と分類され、再利用されること）・malformed（`/Flags` が存在しない object を指す dangling indirect reference が `unknown` へ fail closed **ではなく** 現行互換の BIZ UDゴシックへ戻ること）を確認します。既存の baseline（indirect FontDescriptor + Serif bit の有無、FontDescriptor 無し）は `test/fallback-font-classification.test.js` が既に確認済みで変更していません
- **既存 CI（`ci.yml`）へ追加の workflow ファイルを増やさず、既存の `npm test`（`test/*.test.js`）にそのまま組み込まれます。** 新しい font fixture の取得は不要です（既存の `npm run test:font` が取得する BIZ UDGothic/BIZ UDMincho をそのまま使用）。Node tests は 474 件 → 489 件（+15）、no-Web-Crypto 実行・browser smoke test を含め skip 0 を維持しています
- **公開 API の形は変更していません。** `setFallbackFont()`・`setFallbackFonts()`・`searchText()`・`checkTextMatchReplacement()`・`replaceTextMatch()`・`save()`・`ENGINE_VERSION` は無変更です。新しい公開 error code も追加していません。追加依存もありません
- **bundle size は約 +1KB**（`dist/idontlovepdf-engine.js`: 529.0kb → 531.5kb、`classifyFontResourceDetailed()` と `nestedDictionaryText()` の import 分）
- **font subsetting は今回の対象外です。** `令和 → しょ` で fallback font（BIZ UD明朝）全体を埋め込むため、`22550.pdf` は約 600KB → 約 4.5MB（実測 615,690 → 4,562,587 bytes）へ増加します。これは v0.4.0 以来の既知の制約で、Serif/Sans 判定とは独立した課題として残ります

## v0.5.0 - PoC: BIZ UD明朝/BIZ UDゴシックの自動選択

- **v0.4.4 まで残っていた課題（fallback font の書体差）に対する PoC です。** 元 PDF の font が明朝系でも、fallback 置換箇所は常に BIZ UDGothic（Gothic 系）で描画され、書体差が目立つ、という課題に対し、元 font を Serif/Sans/Unknown の 3 値で判定し、Serif なら BIZ UD明朝、それ以外（Sans または判定不能）なら BIZ UDゴシックを自動選択する仕組みを追加しました。**本体 `idontlovepdf` への取り込みはまだ行っていません。**
- **判定は PDF 自身の `FontDescriptor` `/Flags`（PDF 32000-1:2008, 9.8.2, Table 123, bit 2 = Serif）のみから行います。** font 名の文字列マッチ（`Mincho` を含む・`Gothic` を含む等）は今回導入していません。`/Flags` が Serif bit を立てていれば `serif`、読めて非ゼロなら `sans`、`FontDescriptor` が無い・`/Flags` が読めない・`0` の場合は `unknown` として扱い、`unknown` は常に `sans`（従来どおり BIZ UDゴシック）にフォールバックします
- **`src/font-classification.js` を追加しました。** simple font は自身の `/FontDescriptor` を、Type0/CIDFont は `font-metrics.js` の `resolveDescendantFont()`（indirect / inline `/DescendantFonts` を既に統一的に解決している既存 helper）で辿った descendant font の `/FontDescriptor` を見ます。`/Flags` の direct/indirect 読み取りも `font-metrics.js` の既存 `resolvedNumber()` を再利用し、新しい PDF parser は追加していません
- **新しい公開 API `await editor.setFallbackFonts({ sans?, serif? })` を追加しました。** `await editor.setFallbackFont(fontBytes)` は変更しておらず、内部的には `setFallbackFonts({ sans: fontBytes })` の糖衣構文です。そのため **`setFallbackFont()` のみを呼ぶ既存利用側は、元 PDF の font 判定結果に関係なく常にその 1 font が使われ、v0.4.4 までと完全に同じ挙動を維持します**。どの fallback font を使うかは常に engine 内部（match ごとに元 font resource を判定）が決め、利用側が font を選ぶ API にはしていません
- **fallback font の埋め込み・resource 登録を、1 文書 1 font から 1 文書複数 font（role ごと）へ最小拡張しました。** 内部状態を `fallbackFont`/`fallbackEmbedding`（単数）から `fallbackFonts`/`fallbackEmbeddings`（role キーの Map）へ変更し、ページの `/Font` resource 名の重複回避は role をまたいで共有する `fallbackPageResources`（`pageResourcesObjectNumber -> Map(fontDigest -> resourceName)`）に一本化しました。同じ PDF に明朝系・Gothic 系の置換箇所が両方あっても、BIZ UD明朝・BIZ UDゴシックはそれぞれ 1 回だけ埋め込まれ、resource 名は衝突しません。同一 page の `/Font` を、既に別 font が commit 済みの状態からさらに拡張する場合（2 つ目の fallback font を同じページへ登録する場合）は、`editor.document`（元ファイル）ではなく `editor.pendingObjects` に積まれた直前の commit 結果を土台にして書き換えるよう `registerFallbackResource()` を修正しています（さもないと 2 つ目の font の登録が 1 つ目の登録を上書きしてしまうため）
- **glyph 幅計測・`TJ` adjustment・overflow 判定（`fallback-replacement-overflows-slot` / `fallback-replacement-slot-unknown`）・character/word spacing 判定・writing mode 判定・multi-run 安全判定・variable-length 安全判定・save → reopen・fail closed・atomic replacement は一切変更していません。** どちらの fallback font が選ばれた場合も、そのfont から実際に glyph 幅を取得し、v0.4.4 と同じ安全性判定を通します。「明朝なら幅が近いはず」といった推測は行っていません
- **診断用に `diagnoseFallbackFontSelection(editor, matchId)` を `src/pdf-document.js` に追加しました**（`diagnoseFontMetrics()` と同様、`index.js` からは export しない開発者・テスト向け helper）。source font の名前・Serif/Sans/Unknown 分類・選択された role（`sans`/`serif`）を返し、テストから「実際に BIZ UD明朝が選ばれたか」を engine 自身の判断として確認できます
- **BIZ UD明朝を第二の test fixture font として追加しました。** [BIZ UDMincho](https://github.com/googlefonts/morisawa-biz-ud-mincho) Regular 1.06（SIL Open Font License 1.1）、`scripts/fetch-test-font.js` の `TEST_FONT_SERIF` に URL・SHA-256 を pin しています。BIZ UDGothic 側は無変更です。engine 本体・bundle には同梱せず、テスト取得方法も既存の BIZ UDGothic と同じ（CI 実行時に pinned URL から取得し SHA-256 を検証）です。runtime に外部ネットワーク取得を追加する構成にはしていません
- **`test/fallback-font-classification.test.js` を追加しました。** Serif fixture（`FontDescriptor` に Serif flag、`令和 → しょ` で BIZ UD明朝が選択され埋め込まれ、BIZ UDゴシックは埋め込まれないこと）・Sans fixture（Serif flag 無しの `FontDescriptor`、BIZ UDゴシックのみ埋め込まれること）・Unknown fixture（`FontDescriptor` 無し、従来どおり BIZ UDゴシックへフォールバックすること）・`setFallbackFont()` のみを使った場合の後方互換性・1 PDF に明朝/Gothic 混在（両 font がそれぞれ 1 回だけ埋め込まれ、save → reopen 後も検索・再利用できること、2 つ目の fallback font を同一ページへ登録しても resource 名が衝突しないこと）を確認します
- **v0.4.4 までの回帰テストはすべて維持し、変更していません。** `test/fallback-font.test.js`・`test/fallback-font-tj.test.js`・`test/fallback-font-overflow.test.js`・`test/fallback-font-no-subtle.test.js`・`test/browser/fallback-font.test.js` を含む既存 test は、内部実装の変更（`fallbackFont` → `fallbackFonts` 等）に伴い 2 箇所のみ内部状態への参照を書き換えていますが（`editor.fallbackEmbedding.resources` → `editor.fallbackPageResources`、`editor.fallbackEmbedding` → `editor.fallbackEmbeddings.size`）、安全性の assertion 自体はどちらも削除・緩和していません
- 公開 API の形は `setFallbackFonts()` の追加のみです。`setFallbackFont()`・`searchText()`・`checkTextMatchReplacement()`・`replaceTextMatch()`・`save()`・`ENGINE_VERSION` の形は変更していません。新しい公開 error code も追加していません（`FALLBACK_FONT_INVALID`・`FALLBACK_FONT_ALREADY_IN_USE` を role ごとに判定するようになっただけです）。追加依存もありません
- **bundle size は約 +8KB**（`dist/idontlovepdf-engine.js`: 520.7kb → 529.0kb、`src/font-classification.js` と役割ごとの状態管理の追加分）。font bytes は engine bundle へ直接埋め込まず、従来どおり呼び出し側が `Uint8Array` 等で渡す方式のままです。PDF サイズの増加量は、Gothic のみ使用時 約 +2.9MB（圧縮後）、Mincho のみ使用時 約 +3.8MB（圧縮後、Mincho の方がやや大きい font のため）、両方使用時は約 +6.6MB（ほぼ両者の合計）でした（合成 fixture、1 font あたり 1 文字の置換で計測。font subsetting は今回の対象外です）
- **PR #30 のレビューで指摘された 3 件を修正しました（初回コミット時点にはこれらが残っていました）。**
  1. **fallback font 自身の FontDescriptor に Serif bit が無く、埋め込んだ BIZ UD明朝が save → reopen 後に自分自身を `sans` と誤判定していました。** `buildFallbackFontObjects()`（`src/fallback-font.js`）が書く `/Flags` を、埋め込む role（`sans`/`serif`）に応じて `4`（Symbolic のみ）と `6`（Symbolic + Serif）で書き分けるようにしました。これにより、明朝 → BIZ UD明朝 → 保存 → 開き直し → 再度 `setFallbackFonts()` → 検索 → `diagnoseFallbackFontSelection()` が `classification: "serif"`, `selectedRole: "serif"` を返し、再編集でも BIZ UDゴシックへ落ちずに BIZ UD明朝を再利用することを `test/fallback-font-classification.test.js` で確認しました
  2. **`setFallbackFonts({ serif })` を `serif` だけで呼んだ場合、`sans`/`unknown` と判定された文字にまで BIZ UD明朝が使われてしまう実装になっていました。** ドキュメント（README・型定義）の「`unknown` は常に `sans` へフォールバック」という説明と矛盾していたため、**`sans` を `serif` より先に（または同じ呼び出しで）登録することを必須にしました**。満たされない場合は `FALLBACK_FONT_INVALID` で拒否します。あわせて、1 回の呼び出しで `sans`/`serif` 両方を渡した場合を all-or-nothing にしました（`sans` が既に使用済みで拒否される場合、同じ呼び出しの `serif` も登録されません）。`selectFallbackFont()` はこの「`fallbackFonts` が空でなければ `sans` は必ず登録済み」という不変条件に依存する形へ簡略化しています
  3. **`sans`・`serif` へ同一 font program（同一 SHA-256 digest）を渡せてしまうと、内部状態が破綻する可能性がありました。** ページ resource の重複登録防止（`fallbackPageResources`）は digest 単位で共有されているのに対し、fallback font の embedding 自体（`fallbackEmbeddings`）は role 単位で別々に Type0/CIDFont/ToUnicode の object 番号と glyph 集合を持つため、同一 digest を 2 role へ登録すると、2 つ目の role の置換がページ上では 1 つ目の role の（extendされていない）resource をそのまま参照してしまい、`/W` や ToUnicode に一部 glyph が反映されない不整合が起こり得ました。`assertFallbackDigestsDistinct()` を追加し、`setFallbackFont()` / `setFallbackFonts()` のどちらの経路でも、`sans` と `serif` が同一 digest になる登録（同じ呼び出し内・別の呼び出しでの後追い登録のいずれも）を `FALLBACK_FONT_INVALID` で拒否するようにしました
- **既知の非 blocker（今後の実 PDF 対応で拡張予定）:** `font-classification.js` の `fontDescriptorOf()` は現在 `/FontDescriptor N G R` の indirect reference のみを辿り、`/FontDescriptor << ... >>` の inline（direct）dictionary には対応していません。安全性には影響せず（`unknown` として現行の BIZ UDゴシックへフォールバックするだけ）、実 PDF で遭遇した場合の互換性拡張として残しています

...
- **`sans` は `serif` より先に（または同じ呼び出しで）登録されている必要があります。** `serif` 単独では使えません。`sans`/`unknown` と判定された文字は常に `sans` へフォールバックする設計のため、`sans` が無いと `serif` にはフォールバック先がなくなってしまうからです。満たされていない場合は `FALLBACK_FONT_INVALID` で拒否します（`setFallbackFonts({ serif })` だけを呼ぶことはできません。先に `setFallbackFont()` か `setFallbackFonts({ sans })` を呼んでください）。1 回の呼び出しで両方渡す場合は all-or-nothing で、片方が拒否されればもう片方も登録されません

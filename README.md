...
- **暗号化PDFの診断は`src/encryption.js`が担い、認証・復号は`src/security/`配下が担います。**両者は責任を分離しています: `src/encryption.js`はEncrypt辞書を読み取って`{ filter, version, revision, lengthBits, cryptFilters, permissions, estimatedMethod, ... }`を返すだけで、鍵やパスワードには一切触れません。`src/security/decrypt.js`がこの診断を使って対応範囲かどうかを判定し、対応範囲内だけ実際の認証・復号を行います。
  ...
  - **Encrypt辞書直下の`/Length`・`/V`・`/R`は、nested `/CF`辞書の内部を無視してdepth 1（Encrypt辞書自身の直下）だけを読みます**（`src/pdf-dictionary-text.js`の`topLevelInteger()`/`topLevelValueOffset()`）。Crypt Filter辞書（`/CF << /StdCF << ... /Length 32 >> >>`）は仕様上`/Length`という同じkey名を持つため、辞書全体を単純に正規表現で検索すると、raw byte列の中でどちらが先に現れるかによって誤った方の`/Length`（bytes単位のCrypt Filter鍵長）をEncrypt直下の値（bits単位）として誤取得することがあります（実PDFで実際に発生）。この新しいreaderは単なる`<<`/`>>`・`[`/`]`・literal string・hex stringの深さ追跡だけでなく、辞書を「key → valueを1個skip → 次のkey → valueを1個skip …」という構造として読みます（`skipOneValue()`が辞書・配列・literal string・hex string・name・数値・間接参照`N G R`のいずれか1個の値だけを正しく読み飛ばします）。これにより、`<< /Foo /Length /Length 256 >>`のように、あるkeyのvalueとして`/Length`という名前が現れても、それをkeyと誤認せず正しく後方の実際の`/Length 256`まで到達します。値の整数変換（`topLevelInteger()`）も、value位置の先頭数字だけを読むのではなく、token全体を`parseStrictInteger()`でPDF整数として厳密に検証します（`256foo`や`6.5`は拒否、`+256`は正しく`256`として受理）。既存の`src/content-stream.js`の`skipDictionary()`/`skipArray()`・`readLiteral()`/`readHex()`を再利用しており、PDF dictionary parser全体を新たに書き起こしてはいません。Crypt Filter自身の`/Length`（bytes単位）は、`parseCryptFilters()`が各filterのsub-dictionary textを先に切り出してから読むため、従来どおり正しく取得できます（この2つの`/Length`は常に別々に取得されます）。
...
- `topLevelInteger()`/`topLevelValueOffset()`が、Encrypt辞書直下の`/Length`とnested `/CF`辞書内の`/Length`を、raw byte列内の出現順序（CFが先・Encrypt直下`/Length`が先のどちらでも）に関わらず正しく区別できること。literal string・hex string・array内に`/Length`という文字列やkeyが含まれていても誤認しないこと。Crypt Filter自身の`/Length`は従来どおり`parseCryptFilters()`経由で取得できること。`<< /Foo /Length /Length 256 >>`のように、あるkeyのvalueとして目的のkey名と同じ名前が現れても、それをkeyと誤認せず後方の実際のkeyのvalueへ到達すること（key/value alternationの回帰テスト）。間接参照`N G R`もvalueとして正しく1個だけ読み飛ばせること。整数値の検証が`256`・`+256`は受理し、`256foo`・`256.0`・`6abc`は`null`として拒否する厳密なPDF整数token検証になっていること（先頭の数字だけを読んで切り詰めない）（`test/pdf-dictionary-text.test.js`）
